...
void _showCustomRangeDialog(BuildContext context) {
  DateTime? initialStartDate = _customStartDate ?? _selectedMonth;
  DateTime? initialEndDate = _customEndDate ?? DateTime(_selectedMonth.year, _selectedMonth.month + 1);

  debugPrint('🔵 [DEBUG] _showCustomRangeDialog chamado');
  debugPrint('🔵 [DEBUG] initialStartDate: $initialStartDate');
  debugPrint('🔵 [DEBUG] initialEndDate: $initialEndDate');

  showDialog(
    context: context,
    builder: (context) {
      DateTime? currentStartDate = initialStartDate;
      DateTime? currentEndDate = initialEndDate;
      
      debugPrint('🟡 [DEBUG] Dialog builder executado');
      debugPrint('🟡 [DEBUG] currentStartDate: $currentStartDate');
      debugPrint('🟡 [DEBUG] currentEndDate: $currentEndDate');

      return StatefulBuilder(
        builder: (context, setDialogState) {
          debugPrint('🟠 [DEBUG] StatefulBuilder rebuild');
          ...
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                _DatePickerCard(
                  ...
                  onDateSelected: (date) {
                    debugPrint('🟢 [DEBUG] Data inicial selecionada: $date');
                    setDialogState(() {
                      debugPrint('🟢 [DEBUG] setDialogState - atualizando startDate');
                      currentStartDate = date;
                      // Se a data final for anterior à nova data inicial, ajusta a data final
                      if (currentEndDate != null && currentEndDate!.isBefore(date)) {
                        debugPrint('🟡 [DEBUG] Ajustando endDate para após startDate');
                        currentEndDate = date.add(const Duration(days: 1));
                      }
                      debugPrint('🟢 [DEBUG] Novo currentStartDate: $currentStartDate');
                      debugPrint('🟢 [DEBUG] Novo currentEndDate: $currentEndDate');
                    });
                  },
                ),
                ...
                _DatePickerCard(
                  ...
                  date: currentEndDate,
                  onDateSelected: (date) {
                    debugPrint('🔵 [DEBUG] Data final selecionada: $date');
                    setDialogState(() {
                      debugPrint('🔵 [DEBUG] setDialogState - atualizando endDate');
                      currentEndDate = date;
                      debugPrint('🔵 [DEBUG] Novo currentEndDate: $currentEndDate');
                    });
                  },
                ...
                if (currentStartDate != null && currentEndDate != null) ...[
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'Período: ${DateFormat('dd/MM/yy').format(currentStartDate!)} - ${DateFormat('dd/MM/yy').format(currentEndDate!)}',
                            style: TextStyle(
                              fontSize: 12,
                  ...
                  if (currentEndDate!.isBefore(currentStartDate!)) ...[
                    ...
                    Text(
                      '⚠️ Data final deve ser após a data inicial',
                      style: TextStyle(
                        fontSize: 12,
            ...
            actions: [
              TextButton(
                onPressed: () {
                  debugPrint('🔴 [DEBUG] Cancelar pressionado');
                  Navigator.of(context).pop();
                },
                child: const Text('Cancelar'),
              ),
              ElevatedButton(
                onPressed: (currentStartDate != null && currentEndDate != null && currentEndDate!.isAfter(currentStartDate!))
                    ? () {
                        debugPrint('🟢 [DEBUG] Aplicar pressionado');
                        debugPrint('🟢 [DEBUG] StartDate a aplicar: $currentStartDate');
                        debugPrint('🟢 [DEBUG] EndDate a aplicar: $currentEndDate');
                        _applyCustomRange(currentStartDate!, currentEndDate!);
                        Navigator.of(context).pop();
                      }
                    : null,
...
void _applyCustomRange(DateTime startDate, DateTime endDate) {
  debugPrint('🟠 [DEBUG] _applyCustomRange chamado');
  debugPrint('🟠 [DEBUG] startDate: $startDate');
  ...
  debugPrint('🟠 [DEBUG] Período aplicado com sucesso');
  ...
  Widget build(BuildContext context) {
    ...
    debugPrint('🔵 [DEBUG] _ReportsScreenState build executado');
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'Relatórios',
...
class _PeriodMenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<String>(
      ...
      onSelected: (value) {
      debugPrint('🟡 [DEBUG] Menu item selecionado: $value');
        final now = DateTime.now();
        switch (value) {
          case 'current_month':
          debugPrint('🟢 [DEBUG] Selecionado: Este mês');
            onMonthSelected(DateTime(now.year, now.month));
            break;
          case 'previous_month':
          debugPrint('🟢 [DEBUG] Selecionado: Mês anterior');
            onMonthSelected(DateTime(now.year, now.month - 1));
            break;
          case 'three_months_ago':
          debugPrint('🟢 [DEBUG] Selecionado: Há 3 meses');
            onMonthSelected(DateTime(now.year, now.month - 3));
            break;
          case 'custom':
          debugPrint('🟢 [DEBUG] Selecionado: Personalizado - chamando onCustomRangeSelected');
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        ...
        onTap: () async {
          debugPrint('🟣 [DEBUG] _DatePickerCard $title clicado');
          debugPrint('🟣 [DEBUG] Data atual: $date');
          ...
          try {
            ...
            debugPrint('🟣 [DEBUG] DatePicker retornou: $picked');
            
            if (picked != null) {
              debugPrint('🟣 [DEBUG] Chamando onDateSelected com: $picked');
              onDateSelected(picked);
            } else {
              debugPrint('🟣 [DEBUG] DatePicker cancelado');
            }
          } catch (e, stack) {
            debugPrint('🔴 [DEBUG] ERRO no DatePicker: $e');
